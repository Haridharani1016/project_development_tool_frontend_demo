  ...
  return (
    <div className='py-5'>
      ...
      <ul className='border-r border-gray-300 pr-2'>
        <Link to="/dashboard" onClick={(e) => handleLocation(e)}>
          <li className={`px-5 py-1.5 mb-1 text-gray-600 font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/80 ${paramsWindow === 'dashboard' && 'text-indigo-600 bg-indigo-200/80'}`}>
            Dashboard
          </li>
        </Link>
        <Link to="/projects" onClick={(e) => handleLocation(e)}>
          <li className={`px-5 py-1.5 mb-1 text-gray-600 font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/80 ${paramsWindow === 'projects' && 'text-indigo-600 bg-indigo-200/80'}`}>
            Projects
          </li>
        </Link>
        <Link to="/analytics" onClick={(e) => handleLocation(e)}>
          <li className={`px-5 py-1.5 mb-1 text-gray-600 font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/80 ${paramsWindow === 'analytics' && 'text-indigo-600 bg-indigo-200/80'}`}>
            Analytics
          </li>
        </Link>
        {projects.map((project, index) => (
          <Link key={index} to={`/project/${project._id}`} onClick={(e) => handleLocation(e)}>
